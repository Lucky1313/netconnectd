...
class InstallArchCommand(Command):
    description = "install extras for Arch like service and config files"
    user_options = [("force", "F", "force overwriting files if they already exist")]
    ...
    def run(self):
        global ARCH_FILES, EXTRAS_FOLDERS
        import shutil
        import os
        ...
        for target, files in ARCH_FILES:
            for entry in files:
                extra_tuple = get_extra_tuple(entry)
                if extra_tuple is None:
                    print("Can't parse entry for target %s, skipping it: %r" % (target, entry))
...
class UninstallArchCommand(Command):
    description = "uninstall extras for Arch like service and config files"
    user_options = []
...
def get_cmdclass():
    ...
    cmdclass.update({
        ...
        'install_arch': InstallArchCommand,
        'uninstall_arch': UninstallArchCommand
    })
    return cmdclass
